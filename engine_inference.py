...
def evaluate(
    ...
    # Storage for per-image predictions with bboxes
    per_image_predictions = []
    image_id_to_filename = {}

    # Get image filenames from COCO dataset
    if hasattr(data_loader.dataset, 'coco'):
        coco_dataset = data_loader.dataset.coco
        for img_id in data_loader.dataset.ids:
    ...
    for samples, targets in metric_logger.log_every(
        ...
        # Handle empty labels (images with no annotations)
        input_captions = []
        for target in targets:
            if len(target["labels"]) > 0:
                input_captions.append(cat_list[target["labels"][0]] + " .")
            else:
                # Default to first category if no labels
                input_captions.append(cat_list[0] + " .")
        ...
        # Save per-image predictions with bounding boxes
        for i, (tgt, result) in enumerate(zip(targets, results)):
            image_id = tgt["image_id"].item()

            # Get predicted boxes and scores
            pred_boxes = result["boxes"].cpu().numpy().tolist()  # [N, 4] in x1,y1,x2,y2
            pred_scores = result["scores"].cpu().numpy().tolist()  # [N]
            pred_labels = result["labels"].cpu().numpy().tolist()  # [N]

            # Get ground truth boxes
            gt_boxes = tgt["boxes"].cpu().numpy().tolist()  # [M, 4]
            gt_labels = tgt["labels"].cpu().numpy().tolist()  # [M]

            # Get exemplars
            exemplars_list = tgt["exemplars"].cpu().numpy().tolist()  # [3, 4]

            # Get image size
            orig_size = tgt["orig_size"].cpu().numpy().tolist()  # [H, W]

            # Get category caption
            caption = input_captions[i].replace(' .', '').strip()

            per_image_predictions.append({
                'image_id': image_id,
                'file_name': image_id_to_filename.get(image_id, f'image_{image_id}.jpg'),
                ...
                'category': caption,
                'abs_error': abs(len(pred_boxes) - len(gt_boxes)),
                'rel_error': abs(len(pred_boxes) - len(gt_boxes)) / max(len(gt_boxes), 1) * 100
            })
    ...
    # Save predictions with bounding boxes to JSON
    if args.output_dir and len(per_image_predictions) > 0:
        import json
        ...
        predictions_file = output_dir / 'predictions_with_boxes.json'

        # Calculate summary
        total_images = len(per_image_predictions)
        mae = sum(p['abs_error'] for p in per_image_predictions) / total_images
        avg_rel_err = sum(p['rel_error'] for p in per_image_predictions) / total_images

        output_data = {
            'summary': {
                'total_images': total_images,
                'mae': float(mae),
                'avg_relative_error': float(avg_rel_err),
                'total_gt_count': sum(p['gt_count'] for p in per_image_predictions),
                'total_pred_count': sum(p['pred_count'] for p in per_image_predictions)
            },
            'predictions': per_image_predictions
        }

        with open(predictions_file, 'w') as f:
            json.dump(output_data, f, indent=2)

        print(f"\n✅ Predictions with bounding boxes saved to: {predictions_file}")
